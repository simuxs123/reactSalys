...
function Pagination({currentPage,countriesPerPage,totalCountries,paginate}) {
    const pageNumbers=[];
    const [startIndex,setStartIndex]=useState(0);
    const [endIndex,setEndIndex]=useState(5)
    for (let i=1; i<=Math.ceil(totalCountries/countriesPerPage);i++)//puslapiu numeravimas
        pageNumbers.push(i)
